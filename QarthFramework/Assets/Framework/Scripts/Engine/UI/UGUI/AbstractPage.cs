...
namespace Qarth
{
    public class AbstractPage : AbstractUIElement
    {
        ...
        public UnityEngine.Object LoadPageRes(string resName)
        {
            if (m_GlobalResLoader == null)
            {
                m_GlobalResLoader = ResLoader.Allocate("AbstractPage");
            }
            return m_GlobalResLoader.LoadSync(resName);
        }

        public void ReleasePageRes(string resName)
        {
            if (m_GlobalResLoader != null)
            {
                m_GlobalResLoader.ReleaseRes(resName);
            }
        }
